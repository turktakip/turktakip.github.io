...
class ControllerExtensionPaymentPaytrCheckout extends Controller {
    ...
    public function index()
    {
        ...
        $data['callback_ok'] = $this->config->get('payment_paytr_checkout_callback_page');

		$data['iframe_token'] = $this->gettoken();

        return $this->load->view('extension/payment/paytr_checkout', $data); 
    }
        ...
        if ( !empty( $this->category_full[ $category_id ] ) AND array_key_exists( $this->category_full[ $category_id ], $this->category_installment ) ) {
            ...
            foreach ( $this->category_full as $id => $parent ) {
                if ( $category_id == $id ) {
                    ...
                    else { $return = $this->cat_search( $parent ); }
                } else {
                    $return = 0;
                }
            }
    ...
    public function gettoken()
    {
        ...
        $this->session->data['payment_method']['code'] == 'paytr_checkout';
        ...
        if ( $this->config->get('payment_paytr_checkout_installment_number') != 13 ) {

            foreach( $products as $pro ) {
                $user_basket[] = array( $pro['name'], $pro['total'], $pro['quantity'] );
            }

            $merchant['max_installment']    = in_array( $this->config->get('payment_paytr_checkout_installment_number') , range( 0, 12 ) ) ? $this->config->get('payment_paytr_checkout_installment_number') : 0;

        } else {

            $installment = array();

            $this->category_installment = $this->config->get('payment_paytr_checkout_category_installment');
        ...
        $merchant['oid']                = uniqid().'PAYTROC3'.$order_info['order_id'];
        ...
        if ( $this->config->get('payment_paytr_checkout_lang') == 0 ) {
            ...
            $post_vals['lang'] = ( $this->config->get('payment_paytr_checkout_lang') == 2 ? 'en' : 'tr' );
        }
	...
	public function callback()
    {
        ...
        if ( !isset( $_POST ) ) { echo 'Lutfen URL bilgisini ve protokolu kontrol edin.'; exit; }

        $hash = base64_encode(hash_hmac('sha256', $_POST['merchant_oid'] . $merchant['salt']. $_POST['status']. $_POST['total_amount'], $merchant['key'], true));

        if ( $hash != $_POST['hash'] ) { die('PAYTR notification failed: bad hash'); }

        $order_id = explode( 'PAYTROC3', $_POST['merchant_oid'] );
        $this->load->model('checkout/order');
        ...
        if ( $getOrder ) {
            ...
            if ( $_POST['status'] == 'success' AND $getOrder['order_status_id'] == 0 ) {
                ...
                $this->model_checkout_order->addOrderHistory( $order_id[1], $this->config->get('payment_paytr_checkout_order_completed_id'), $note, true );
                ...
                $this->model_checkout_order->addOrderHistory( $order_id[1], $this->config->get('payment_paytr_checkout_order_canceled_id'), $note, true );
