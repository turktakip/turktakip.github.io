			...
			<ul class="posts">
				<?php while ( $r->have_posts() ) : $r->the_post();
					...
					if ( has_post_thumbnail( get_the_ID() ) ) {
						...
						if ( ! empty ( $thumb ) ) {
							$image = '<a href="' . get_permalink() . '" class="hoverBorder pull-left">'.$thumb.'</a>';
						}
					}
					?>
					<li class="lp-post"><?php echo $image; ?>
						<h4 class="title">
						...
						<?php if($showExcerpt == 'show' ) : ?>
							<div class="text"><?php echo $the_str . '...'; ?></div>
						<?php endif; ?>
						<?php if($showCommentCount == 'show' ) : ?>
							<div class="lp-post-comments-num"><?php echo $cCount.' '.__('comments', 'zn_framework');?></div>
						<?php endif; ?>
					</li>
				<?php endwhile; ?>
			</ul>
