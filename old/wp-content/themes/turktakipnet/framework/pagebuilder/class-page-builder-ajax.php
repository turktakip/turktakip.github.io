...
add_action('wp_ajax_znpb_do_save_element', 'znpb_do_save_element');
...
function znpb_render_module() {
	...
	if( !empty( $_POST['template_name'] ) ) {
		...
		$template_data = ZN()->pagebuilder->templates->zn_pb_get_templates( 'zn_pb_el_templates', $_POST['template_name'] , '=' );
...
function znpb_publish_page() {
	...
	//update_post_meta( zn_get_the_id(), 'zn_page_builder_els',  $element_data );
	...
	if( $has_page_options ) {
		...
		foreach( $page_options as $element )
		{

			if ( isset ( $_POST['page_options'] ) ) {
				$params = array();
				parse_str( $_POST['page_options'] , $params);

				if ( isset( $params[$element['id']] ) ) {
					update_post_meta( zn_get_the_id(), $element['id'], $params[ $element['id'] ] );
				}
...
function znpb_do_save_element(){
	...
	{
		$return['message'] = 'This element name is already in use. Please delete the old saved element before saving this one or use a different element name';
...
function zn_save_template(){
	...
	if(!empty($template_name_check))
	{
		...
		$return['message'] = 'This template name is already in use. Please delete the old template before saving this one or use a different template name';
