...
class TH_FlexSlider extends ZnElements
{
    ...
    function scripts(){
        // LOAD CSS AND JS
        wp_enqueue_style( 'flex_slider_styles', THEME_BASE_URI . '/sliders/simple_slider/simpleslider.css', false, ZN_FW_VERSION );
    ...
    function css(){
        $css = '';
        $uid = $this->data['uid'];
        $height = $this->opt('fs_height', '470');

        // No need to add the css code if the value is left default which is 600px
        if( $height != '470' ){
            $css .= '.'.$uid.' .zn_simple_slider-itemimg, .'.$uid.' .zn_simple_slider_container {height: '. $height.'px;} ';
        }
    ...
    function element()
    {
        ...
        ?>
        <div class="kl-slideshow simpleslider__slideshow <?php echo $this->data['uid']; ?> <?php echo $style; ?> <?php echo $bm_class ?> <?php echo $this->opt('css_class',''); ?>">
            ...
            <div class="kl-slideshow-inner container kl-slideshow-safepadding">
                <div class="row">
                    <div class="col-sm-12">
                        <div class="zn_simple_slider_container showOnMouseover <?php echo $thumbs; ?> <?php echo $fs_shadow;?> kl-flex--<?php echo $sliderStyle; ?>">
                                ...
                                if ( isset ( $options['single_flex'] ) && is_array( $options['single_flex'] ) ) {
                                    foreach ( $options['single_flex'] as $slide ) {
                                        ...
                                        if ( isset ( $slide['fs_slide_link']['url'] ) && ! empty ( $slide['fs_slide_link']['url'] ) ) {
                                            // Set defaults
                                            $link_start = '<a class="slide__link" href="' . $slide['fs_slide_link']['url'] . '" target="' . $slide['fs_slide_link']['target'] . '">';
                                            $link_end   = '</a>';
                                            if(!empty($slide['fs_slide_link']['title'])) {
                                                $link_title   = $slide['fs_slide_link']['title'];
                                            }
                                        }
                                        ...
                                        if ( isset ( $slide['fs_slide_title'] ) && ! empty ( $slide['fs_slide_title'] ) ) {
                                            echo '<h2 class="flex-caption">' . $slide['fs_slide_title'];
                                        }
                            ...
                            <?php if( $this->opt('fs_bullets', 'true') || $this->opt('fs_show_thumbs', '0') == 1 ){ ?>
                            <div class="zn_simple_carousel-pagi cfs--pagination <?php echo $thumbs ?>"></div>
                            <?php } ?>

                            <?php if($this->opt('fs_nav', 'true')){ ?>
            ...
            'general' => array(
                ...
                'options' => array(
                    array (
                        ...
                        "description" => __( "Add a height for the slider. Empty means automatic height.", 'zn_framework' ),
                        "id"          => "fs_height",
                        "std"         => "470",
                        "type"        => "text",
                        "class"       => "",
           ...
           'help' => array(
                ...
                'options' => array(
                    ...
                    array (
                        ...
                        "description" => __( '<span class="dashicons dashicons-video-alt3 u-v-mid"></span> <a href="http://support.hogash.com/kallyas-videos/#YeqY8rqbI7Q" target="_blank">Click here to access video tutorial for this element.</a>', 'zn_framework' ),
                        "id"          => "video_link",
                        "std"         => "",
                        "type"        => "zn_title",
                    ...
                    array (
                        ...
                        "description" => __( '<span class="dashicons dashicons-format-aside u-v-mid"></span> <a href="http://support.hogash.com/documentation/simple-slider/" target="_blank">Click here to access documentation for this element.</a>', 'zn_framework' ),
                        "id"          => "docs_link",
                        "std"         => "",
                        "type"        => "zn_title",
                        "class"       => "zn_full zn_nomargin"
